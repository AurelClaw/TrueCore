"""
Einfacher Test-Runner für das Cartographer-Modul (ohne pytest)
"""
...
def run_test(test_func, result):
    ...
    try:
        ...
        result.add_fail(test_func.__name__, f"{type(e).__name__}: {e}")
...
def test_node_confidence_clamping():
...
def test_calculate_connection_strength():
...
def test_cartographer_creation():
...
def test_classify_concept_domain():
...
def test_get_or_create_concept_node():
...
def test_get_or_create_period_node():
...
def test_get_or_create_source_node():
...
def test_map_fragments_integration():
...
def test_export_for_visualization():
...
def test_import_excavator_fragment():
    ...
    try:
        ...
        print("    Integration mit Excavator erfolgreich!")
        
    except ImportError:
        print("    ⚠ Excavator-Modul nicht verfügbar (übersprungen)")
...
def main():
    ...
    print("=" * 60)
    print("Knowledge Cartographer Tests")
    print("=" * 60)
    
    result = TestResult()
    
    # Sammle alle Test-Funktionen
    tests = [
        # Node Tests
        test_node_creation,
        test_node_confidence_clamping,
        test_node_to_dict,
        test_node_from_dict,
        
        # Edge Tests
        test_edge_creation,
        test_edge_weight_clamping,
        ...
        test_add_node,
        test_add_edge,
        test_add_edge_invalid_nodes,
        test_remove_node,
        test_get_neighbors,
        test_get_nodes_by_domain,
        test_get_nodes_by_type,
        test_find_path,
        test_find_path_no_connection,
        test_temporal_layers,
        test_calculate_centrality,
        test_calculate_connection_strength,
        test_graph_stats,
        test_serialization,
        
        # KnowledgeCartographer Tests
        test_cartographer_creation,
        test_classify_concept_domain,
        test_get_or_create_concept_node,
        test_get_or_create_period_node,
        test_get_or_create_source_node,
        test_map_fragments_integration,
        test_domain_summary,
        test_temporal_analysis,
        test_export_for_visualization,
        
        # Integration
        test_import_excavator_fragment,
    ]
    
    print(f"\nFühre {len(tests)} Tests aus...\n")
    
    for test_func in tests:
        run_test(test_func, result)
    
    # Zusammenfassung
    print("\n" + "=" * 60)
    print(f"Ergebnisse: {result.passed} bestanden, {result.failed} fehlgeschlagen")
    print("=" * 60)
